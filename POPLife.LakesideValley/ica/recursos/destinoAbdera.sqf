...
if !(isNil {_veh getVariable "cargado"}) exitWith {hint "El camion ya esta cargado"};
	...
	if (_rec == "alimento") then {
		...
		if ((parseNumber _can) <= aliJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de alimento con destino a Abdera...", (parseNumber _can)];
				sleep 4;
				...
				if !(isNil {_veh getVariable "cargado"}) exitWith {hint "El camion ya esta cargado"};
				hint "Todo listo, buen viaje hasta Abdera :)";
				...
				//_caja addAction ["<t color='#DF0101'>Eliminar magicamente</t>",'deleteVehicle (_this select 0)', "", 9,false,true,"","side player == west"];
				...
				_caja addAction["Mirar etiqueta",{hint "Destino: Abdera."},"",0,false,false,"",""];
			
				_caja setVariable ["Abdera", (parseNumber _can), true];	
	...
	if (_rec == "agua") then {
		...
		if !(aguJac > 250) exitWith {hint "El almacen de agua de Kavala tiene que estar como minimo al 25%"};
		...
		if ((parseNumber _can) <= aguJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de agua con destino a Abdera...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
	...
	if (_rec == "medicamentos") then {
		...
		if ((parseNumber _can) <= medJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de medicinas con destino a Abdera...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
	...
	if (_rec == "Combustible") then {
		...
		if ((parseNumber _can) <= comJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de combustible con destino a Abdera...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
