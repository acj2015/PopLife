	...
	if (_rec == "alimento") then {
		...
		if ((parseNumber _can) <= aliJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de alimento con destino a Kalithea...", (parseNumber _can)];
				sleep 4;
				...
				if !(isNil {_veh getVariable "cargado"}) exitWith {hint "El camion ya esta cargado"};
				hint "Todo listo, buen viaje hasta Kalithea :)";
				...
				_caja addAction["Mirar etiqueta",{hint "Destino: Kalithea."},"",0,false,false,"",""];
			
				_caja setVariable ["Kalithea", (parseNumber _can), true];		
	...
	if (_rec == "Agua") then {
		...
		if ((parseNumber _can) <= aguJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de carne con destino a Kalithea...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
	...
	if (_rec == "medicamentos") then {
		...
		if ((parseNumber _can) <= medJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de medicinas con destino a Kalithea...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
	...
	if (_rec == "Combustible") then {
		...
		if ((parseNumber _can) <= comJac) then {
			...
			if (life_cash >= _impali) then {
				...
				hint format ["Empaquetando %1 suministros de combustible con destino a Kalithea...", (parseNumber _can)];
				sleep 4;
				hint "Montando la mercancia en el camion...";
				sleep 4;
				if ((_veh distance (getMarkerPos "ZDKavala")) > 4.4) exitWith {hint "No hay un camion (un un camion apropiado) en la zona de descarga.";};
