...
//Knock out, this is experimental and yeah...
	...
	case 34:
	{	
	        if( (!_shift && !_ctrlKey ) && (!life_action_inUse) && (vehicle player == player) ) then
        {
            {
        ...
        {
            {
                ...
                if(_val > 0 ) then
                {
                    if( _str == "Forzar cerradura" || _str == "lockpick" ) then
                    {
	        ...
	        if(playerSide == independent && (!life_action_inUse) && (vehicle player == player) ) then
        {
            {
                _str = [_x] call life_fnc_varToStr;
                _val = missionNameSpace getVariable _x;
	...
	// ANTI CTRL + ESC
		case 1:
		{
			...
			diag_log format ["SERVIDOR POPLIFE: %1 usando CTRL + ESC Para Desconectar",player getVariable["realname",name player]];
			[[1,format["SERVIDOR POPLIFE: %1 utilise CTRL + ESC Para Desconectar",player getVariable["realname",name player]]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
			SOCK_fnc_updateRequest;
			};
		};
