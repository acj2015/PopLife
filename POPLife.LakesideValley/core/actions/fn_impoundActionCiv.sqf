...
if(!((player distance (getMarkerPos "fourriere1") < 25) || (player distance (getMarkerPos "fourriere2") < 25))) exitWith {hint "Tu dois etre pret d une fourriere"};
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship") || (_vehicle isKindOf "Motorcycle") || (_vehicle isKindOf "A3L_Tahoe_Base")) then
{
	...
	if((count crew _vehicle) == 0) then
	{
		...
		switch (true) do
		{
			case (_vehicle isKindOf "Car"): {if(playerSide == west) then {_price = (call life_impound_car_cop);}else{_price = (call life_impound_car);};};
