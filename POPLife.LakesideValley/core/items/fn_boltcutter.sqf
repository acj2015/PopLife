...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_owner","_ownername"];
_building = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
if(isNull _building) exitWith {};
if(!(_building isKindOf "House_F")) exitWith {hint "You are not looking at a house door."};

if(isNil {(_building getVariable "house_owner")}) exitWith {hint localize "STR_House_Raid_NoOwner"};
_owner = _building getVariable "house_owner";
_ownername = _owner select 1;

if((_building getVariable "alarm")) then {

hint "La casa esta protegida por una alarma ... que acaba de empezar a sonar.";

	_Pos = position player; // by ehno: get player pos
	_marker = createMarker ["CambroMarker", _Pos]; //by ehno: Place a Maker on the map
	"CambroMarker" setMarkerColor "ColorRed";
	"CambroMarker" setMarkerText "/!\ Cambriolage en cours /!\";
	"CambroMarker" setMarkerType "mil_warning";
};
