...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owner"}) then {
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		if(_curTarget getVariable ["alarm",false]) then {
			_Btn4 ctrlSetText "Alarme ON";
		} else {
			_Btn4 ctrlSetText "Alarme OFF";
		};
		_Btn4 buttonSetAction "[life_pInact_curTarget] call life_fnc_alarmHouse; closeDialog 0;";
		_Btn4 ctrlShow true;		
