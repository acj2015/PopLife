...
fnc_pickpumpkin = {
        ...
        if ((_hour <= 5) OR (_hour >= 19)) then {
                if (scarylevel == 0) then {
                        ...
                        candy = candy + _candy;
                        _msgtext1 = format ["Piensa que eres un miedica y solo te dio %1 caramelos!",_candy];
                        [_msgtext1,20,"orange"] call A3L_Fnc_Msg;
                };
        } else {
                _extracandy = round (random 50);
                _candy = 10 + _extracandy;
                _msgtext1 = format ["Has Encontrado %1 Caramelos!",_candy];
...
["Elle a fuit des que vous etes rentre, ne laissant aucun bonbon!",20,"red"] call A3L_Fnc_Msg;
} else {
_extracandy = round (random 100);
_candy = 100 + _extracandy;
candy = candy + _candy;
_msgtext1 = format ["Elle vous a laisse seul avec %1 bonbon!", _candy];
[_msgtext1,20,"green"] call A3L_Fnc_Msg;
};};
[] spawn fnc_endedhunt;
};
...
[_angel1] call fnc_eyecheck;
        ...
        _moveseconds = _moveseconds + 1;
                ...
                if (player distance _angel1 <= 2 and isLookingAt == 1) then {
                        ...
                        titleText ["Te tiene atrapado, ¡CORRE!", "PLAIN DOWN", 1];
                        sleep 1;
                        switch (true) do {
...
_msgtext1 = format ["Has comprado %1 Por %2",_itemname,_price];
[_msgtext1,20,"green"] call A3L_Fnc_Msg;
...
["No dispones de suficientes caramelos!",20,"red"] call A3L_Fnc_Msg;
};};